const AoiError = require("./KvError.js");
const BaseClient = require("./KvBase.js");
const { Command } = require("./Commands.js");
const { FunctionManager } = require("./Functions.js");

//Initialize kv.js Client
class Client extends BaseClient {
    ...
    onMemberAvailable() {
        if (!this.Options.intents.includes("GUILD_MEMBERS"))
    ...
    deletedCommand(d = {}) {
        if (!d.code) {
            throw new TypeError(
                `${d?.name || "bilinmeyen isim"} içinde kod sağlanmamıştır.
    ...
    interactionCommand(d = {}) {
        if (!d.prototype) {
            throw new TypeError(
                `Prototype is not provided in ${d.name || "bilinmeyen isim"
                }: interactionCommand.`,
            );
        }
        if (!d.code) {
