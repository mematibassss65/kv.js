...
class AoiError {
  ...
  static CallbackError(callback, intent, line) {
    const error = new Error(
      `(Eksik Intentler) : "${callback}", "${intent}" intentini gerektirir.`,
    );
    error.name = "CallbackError";
    error.fileName = "./Bot.js";
    error.Satır Numarası = line;
    throw error;
  }
  ...
  static CommandError(command, type, name, position) {
    if (type === "name") {
      const error = new Error(
        `KvError: "${command}" içinde "İsim" özelliği eksik (konum: ${position})`,
      );
      error.name = "CommandNameError";
      throw error;
    } else if (type === "code") {
      const error = new Error(
        `KvError: "${ name || "the Command" }" içinde "Kod" sağlanamaz: ${command} (konum: ${position})`,
      );
      error.name = "CommandCodeError";
      throw error;
    } else if (type === "channel") {
      const error = new Error(
        `KvError: "${ name || "the Command" }" içinde "Kanal" sağlanamaz: ${command} (konum: ${position})`,
      );
  ...
  static functionErrorResolve(d, type, data, message) {
    ...
    switch (type) {
      case "message":
        ans = `\`\`\`js\nKvError: ${d.func}: ${ data.inside || "" } İçinde Geçersiz Mesaj Kimliği Sağlandı\n { \n   Satır Numarası : ${d.funcLine},\n   Satır : \`${
          d.command.codeLines[d.funcLine - 1]
        }\`,\n   Yol : "${d.command.__path__}" \n }\`\`\``;
        break;
      case "channel":
        ans = `\`\`\`js\nKvError: ${d.func}: ${ data.inside || "" } İçinde Geçersiz Kanal Kimliği Sağlandı\n { \n   Satır Numarası : ${d.funcLine},\n   Satır : \`${
          d.command.codeLines[d.funcLine - 1]
        }\`,\n   Yol : "${d.command.__path__}" \n }\`\`\``;
        break;
      case "user":
        ans = `\`\`\`js\nKvError: ${d.func}: ,${ data.inside || "" } İçinde Geçersiz Kullanıcı Kimliği Girildi \n { \n   Satır Numarası : ${d.funcLine},\n   Satır : \`${
          d.command.codeLines[d.funcLine - 1]
        }\`,\n   Yol : "${d.command.__path__}" \n }\`\`\``;
        break;
      case "member":
        ans = `\`\`\`js\nKvError: ${d.func}:${
          data.inside || ""
        } İçinde Geçersiz Üye Kimliği Sağlandı \n { \n   Satır Numarası : ${d.funcLine},\n   Satır : \`${
          d.command.codeLines[d.funcLine - 1]
        }\`,\n   Yol : "${d.command.__path__}" \n }\`\`\``;
        break;
      case "role":
        ans = `\`\`\`js\nKvError: ${d.func}: ${ data.inside || "" } İçinde Geçersiz Rol Kimliği Sağlandı \n { \n   Satır Numarası : ${d.funcLine},\n   Satır : \`${
          d.command.codeLines[d.funcLine - 1]
        }\`,\n   Yol : "${d.command.__path__}" \n }\`\`\``;
        break;
      case "guild":
        ans = `\`\`\`js\nKvError: ${d.func}: ${ data.inside || "" } İçinde Geçersiz Sunucu Kimliği Sağlandı \n { \n   Satır Numarası : ${d.funcLine},\n   Satır : \`${
          d.command.codeLines[d.funcLine - 1]
        }\`,\n   Yol : "${d.command.__path__}" \n }\`\`\``;
        break;
      case "emoji":
        ans = `\`\`\`js\nKvError: ${d.func}: ${ data.inside || ""} İçinde Geçersiz Emoji Kimliği Sağlandı\n { \n   Satır Numarası : ${d.funcLine},\n   Satır : \`${
          d.command.codeLines[d.funcLine - 1]
        }\`,\n   Yol : "${d.command.__path__}" \n }\`\`\``;
        break;
      case "option":
        ans = `\`\`\`js\nKvError: ${d.func}: ${ data.inside || ""} İçinde Geçersiz Seçenek Kimliği Sağlandı\n { \n   Satır Numarası : ${d.funcLine},\n   Satır : \`${
          d.command.codeLines[d.funcLine - 1]
        }\`,\n   Yol : "${d.command.__path__}" \n }\`\`\``;
        break;
      case "custom":
        ans = `\`\`\`js\nKvError: ${d.func}: ${message} ${
          data.inside || ""
        } \n { \n   Satır Numarası : ${d.funcLine},\n   Satır : \`${
          d.command.codeLines[d.funcLine - 1]
        }\`,\n   Yol: "${d.command.__path__}" \n }\`\`\``;
