...
const Interpreter = async (
    ...
    try {
                            ...
                            if (typeof unpacked.inside !== "string") {
                                if (suppressErrors) return suppressErrors;
                                    ...
                                    return client.options.suppressAllErrors
                                        ...
                                        : `\`KvError: ${this.func}: Geçersiz Kullanım! (satır : ${funcLine})\``;
                                }
                            } else return false;
                        },
                        noop() {
                        ...
                        inside(unpacked) {
                            if (typeof unpacked.inside !== "string") {
                                if (suppressErrors) return suppressErrors;
                                    ...
                                    return client.options.suppressAllErrors
                                        ? client.options.errorMessage
                                        : `\`KvError: ${func}: Geçersiz Kullanım! (satır : ${funcLine})\``;
                                }
                            } else return false;
                        },
                        noop() {
        ...
        if (embeds?.some((x) => x === undefined)) {
            error = true;
            return AoiError.consoleError("EmbedError", "Dizinlerin girişi boş");
        }
        if (returnCode) {
